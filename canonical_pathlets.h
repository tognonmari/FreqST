...
//represents a collection of pathlets organized as a tree, based on a subtrajectory taken from a trajectory collection
template<metric_space m_space>
class LightWeightBinaryPathletTree{
    ...
    public:
        LightWeightBinaryPathletTree(){
            
        }
        
        LightWeightBinaryPathletTree(trajectory_t& supp, id_t traj_id, int max_depth, int min_length){
            //this->support = supp;
            this-> n = supp.get_trajectory_size(traj_id);
            this-> l = min_length;
            this-> d = max_depth;
            this->trajectory_id = traj_id;
            //assert(min_length <= n);
