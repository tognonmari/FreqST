...
private:
    ...
    */
    bool find_match_with_pathlet_from_start_vertex(const trajectory_t& sample, vertex* start_vertex, id_t trajectory_id){
        ...
        std::queue<vertex*> current_column;
        std::queue<vertex*> next_column;
        
        next_column.push(start_vertex);
        ...
        while(!next_column.empty() && next_column_idx>leftmost_column){
            ...
            while(!current_column.empty()){
                start_vertex = current_column.front();
                current_column.pop();
                if(next_column.empty()){
                    ...
                    assert(next_column.back()!= nullptr);
        ...
        if(next_column.empty()) {return false;}
