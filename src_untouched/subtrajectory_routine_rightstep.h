...
private:
    ...
    void populate_column(free_space_graph_incremental_t &free_space, const distance_t &distance_max, const index_t &column, bool reverse_column = false){
        ...
        int zeroes = 0;
        if(!reverse_column){
            point_t x_axis_coordinate = trajectory[column];
            for (int i = 0; i<trajectory->total_size(); i++){

                if (trajectory.is_point_deleted(i)) continue;
                auto d_ij = distance_function_t{}(sample[i], x_axis_coordinate); //FI
                if (d_ij <= distance_max ){

                    free_space.add_zero(i);
                    zeroes++;
            ...
            point_t x_axis_coordinate = trajectory[column];
            for(int i = trajectory->total_size()-1; i>=0; i--){
